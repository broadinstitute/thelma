	...
	for attempts := 3; attempts > 0; attempts-- {
		...
		for _, stateRelease := range stateReleases {
			...
			_release := release{
				...
				// helmfileOverlays  - set below
				// releaseType       - set below
			}
			...
			case "cluster":
				...
				_release.helmfileOverlays = []string{} // TODO - should we support overlays for cluster releases?
			...
			_releases[_release.name] = &_release
		}
